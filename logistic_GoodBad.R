...
#-------Importing the data---------
goodbad<-read.csv("C:\\Users\\admin\\Documents\\Jigsaw\\Logistic Regression class files\\GOODBAD.csv")
str(goodbad)
#1=good, 0=bad
...
myresult<-glm(data=train,Good.Bad ~ Duration + Check_Account_Status,
              family=binomial)
...
myresult<-glm(data=train,Good.Bad ~ Check_Account_Status+Duration,
              family=binomial)
summary(myresult)
...
predicted <- predict(myresult,data=train,type="response")

#Confusion Matrix
predbkt<-ifelse(predicted>0.20,'B','G')
